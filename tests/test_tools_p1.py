...
async def test_financial_ratios_happy() -> None:
    ...
    raw = await fundamental.get_financial_ratios("FPT", gateway=gw)
...
async def test_financial_statements_invalid_type() -> None:
    gw = _gw()
    raw = await fundamental.get_financial_statements(
...
async def test_screen_stocks_bad_json() -> None:
    gw = _gw()
    raw = await screening.screen_stocks(filters="not-json", gateway=gw)
    payload = json.loads(raw)
    assert payload["ok"] is False
    assert payload["code"] == "VALIDATION"
...
async def test_screen_stocks_happy_caps_limit() -> None:
    gw = _gw([{"ticker": "AAA"}])
    raw = await screening.screen_stocks(
...
async def test_technical_indicators_happy() -> None:
    gw = _gw([{"rsi": 55}])
    raw = await screening.get_technical_indicators(
        "VNM",
        '[{"name":"rsi","window":14}]',
        gateway=gw,
    )
...
async def test_stock_prices_and_index() -> None:
    gw = _gw([{"close": 100}])
    raw = await market.get_stock_prices("FPT", latest=True, gateway=gw)
    assert json.loads(raw)["ok"] is True
    raw2 = await market.get_index_constituents("vn30", gateway=gw)
...
async def test_basic_info_and_snapshot() -> None:
    gw = _gw({"name": "FPT"})
    raw = await universe.get_basic_info("FPT", gateway=gw)
    assert json.loads(raw)["ok"] is True
    raw2 = await fundamental.get_equity_snapshot("FPT", metrics="pe,pb", gateway=gw)
    assert json.loads(raw2)["ok"] is True
...
async def test_call_method_dry_run() -> None:
    gw = _gw()
    raw = await meta.fiinquantx_call_method(
...
async def test_sdk_error_envelope() -> None:
    ...
    raw = await market.get_market_breadth(gateway=gw)
    payload = json.loads(raw)
    assert payload["ok"] is False
...
@pytest.mark.asyncio
async def test_report_issue_local() -> None:
    raw = await meta.report_issue(
        "why fail?",
        "tool timeout",
    ...
    payload = json.loads(raw)
    assert payload["ok"] is True
    assert payload["data"]["delivery"] == "local_log_only"
    assert "issue_id" in payload["data"]
